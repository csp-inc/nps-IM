...
transect_azimuth_and_centroid <- function(x, y) {
  tibble('transect_azimuth'=atan2(y[2] - y[1], x[2] - x[1]) * 180 / pi,
             'transect_centroid_x'=mean(x),
             'transect_centroid_y'=mean(y),
             'pi_order'=ifelse(y[2]>y[1], 1, -1))
}

# used in transect_point_intercepts, which is commented out in stats.r
...
point_along_transect <- function(data, d=seq(.25, 10, .5)) {
  ...
  origin <- data %>%
    ...
    as.matrix
  origin_mat <- matrix(rep(origin, each=length(d)), nrow=length(d),
...
# in src\stats.r
# this implies to me that just need site centroids
# but the comment suggests that it wants transect centroids and that there's some precursors. which do I need and what are the precursors?
...
get_most_recent_plot_cover_sf <- function(data, macroplots_sf) {
  data %>%
    ...
    summarise(c3_cover=sum(any_is_live_c3_npg)/300 * 100,
