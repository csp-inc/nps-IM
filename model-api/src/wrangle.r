...
get_mod_summary_files <- function(folder, parse_coda_output = FALSE) {
  jags_info_files <-
    list.files(folder,
  ...
  d_coda <- NULL
  if (parse_coda_output) {
    coda_output <- jags_info_df %>%
      # slice(1:5) %>%
      pull(full_path) %>%
      map(function(x) list.files(file.path(x, "01-diagnostics"), pattern = "convergence-diagnostics.txt", full.names = TRUE)) %>%
      map(function(x) {
        if (length(x) > 0) {
          read_table(x, skip = 3, col_names = c("param", "point_est", "upper_ci")) %>%
            summarise(gelman_diag_pt_est = as.numeric(max(point_est)))
        } else {
          NA
        }
      }) #
    d_coda <- as_tibble(do.call(rbind, coda_output), .name_repair = "minimal")
  }
