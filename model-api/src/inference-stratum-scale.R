get_stratum_inference <- function(mcarray_list, data, folder, resp_var_desc,
  ...
  in_and_out <- lapply(strat_mean_suffixes, function(strat_mean_suffix) {
    ...
    d_stratum_mu_subset <- stratum_mu_draws$stratum_mu_subset %>%
      ...
      ungroup() %>%
      left_join(temp, by = c("year", "stratum_id"))
    # write_csv(file.path(
    ...
    if (n_strata > 1) {
      ...
      save_figure(
        ...
        p_width = 4, p_height = 6, device = NULL
      )
    }
