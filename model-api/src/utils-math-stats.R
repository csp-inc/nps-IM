...
get_interval <- function(x, interval, method = c("hdi", "eti"), rm_na = FALSE) {
  if (method[1] == "hdi") {
    hdi(x, credMass = interval)
  } else if (method[1] == "eti") {
    quantile(x,
      probs = c(0 + (1 - interval) / 2, 1 - (1 - interval) / 2),
      na.rm = rm_na
    )
    # summary(x, quantile,
...
get_interval2 <- function(x, interval = 0.89, method = c("hdi", "eti"),
  ...
  if (include_median) {
    out <- c(lower = unname(out[1]), median = median(x), upper = unname(out[2]))
  }
